-- SSDLC Onboarding & Tracking — logical schema, PostgreSQL dialect (reference)
-- Differences from the SQLite dialect, and only these:
--   * ULID identifiers    TEXT          -> VARCHAR(26)
--   * timestamps          TEXT (ISO)    -> TIMESTAMPTZ
--   * booleans            INTEGER 0/1   -> BOOLEAN
--   * decimals            REAL          -> NUMERIC
-- Tables, constraints, partial indexes, views and the seed row are identical by
-- construction.  If this list ever grows, the core schema has drifted out of
-- portable SQL (§9.2).
...
--
-- Conventions
--   * ids are ULIDs stored as VARCHAR(26); never reused, never derived from a source key
...
CREATE TABLE subject (
    ...
    subject_type        TEXT NOT NULL
                        CHECK (subject_type IN ('inventory_record','application',
                                                ...
                                                'build_artifact')),
    source_id           VARCHAR(26) NOT NULL REFERENCES source(id),   -- never null; see header
...
CREATE TABLE source (
    ...
    created_at          TIMESTAMPTZ NOT NULL,
    last_sync_run_id    VARCHAR(26) REFERENCES sync_run(id),
...
CREATE TABLE source_decision_field (
    source_id           VARCHAR(26) NOT NULL REFERENCES source(id),
    field               TEXT NOT NULL,
...
CREATE TABLE decision_packet (
    ...
    source_id           VARCHAR(26) REFERENCES source(id),
...
CREATE TABLE decision (
    ...
    decision_packet_id  VARCHAR(26) REFERENCES decision_packet(id),
    confirmed_by_user   BOOLEAN NOT NULL DEFAULT TRUE,
    superseded_by       VARCHAR(26) REFERENCES decision(id),
...
CREATE TABLE decision_subject (
    decision_id         VARCHAR(26) NOT NULL REFERENCES decision(id),
    subject_id          VARCHAR(26) NOT NULL REFERENCES subject(id),
    decided_against_hash TEXT,                    -- content_hash at decision time
    status              TEXT NOT NULL DEFAULT 'current'
                        CHECK (status IN ('current','stale','superseded')),
    went_stale_at       TIMESTAMPTZ,
...
CREATE TABLE sync_run (
    id                  VARCHAR(26) PRIMARY KEY,
    source_id           VARCHAR(26) NOT NULL REFERENCES source(id),   -- role comes from the source
    mode                TEXT NOT NULL
...
CREATE TABLE sync_op (
    id                  VARCHAR(26) PRIMARY KEY,
    sync_run_id         VARCHAR(26) NOT NULL REFERENCES sync_run(id),
    op                  TEXT NOT NULL
                        CHECK (op IN ('add','update','remove','unchanged')),
    record_key          TEXT NOT NULL,            -- external id at the source
    subject_id          VARCHAR(26) REFERENCES subject(id),   -- resolved row, once known
...
CREATE TABLE inventory_record (
    id                  VARCHAR(26) PRIMARY KEY REFERENCES subject(id),
    source_id           VARCHAR(26) NOT NULL REFERENCES source(id),
    ...
    absent_since        TIMESTAMPTZ,                     -- full/scoped sync, or explicit remove
    ...
    scope_decision_id   VARCHAR(26) REFERENCES decision(id),
    scope_stale         BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE inventory_record_attribute (
    record_id           VARCHAR(26) NOT NULL REFERENCES inventory_record(id),
    key                 TEXT NOT NULL,
...
CREATE TABLE application_alias (
    alias_id            VARCHAR(26) PRIMARY KEY REFERENCES application(id),
    alias_handle        TEXT NOT NULL,
    canonical_id        VARCHAR(26) NOT NULL REFERENCES application(id),
    decision_id         VARCHAR(26) REFERENCES decision(id),
...
CREATE TABLE application_source_link (
    application_id      VARCHAR(26) NOT NULL REFERENCES application(id),
    inventory_record_id VARCHAR(26) NOT NULL REFERENCES inventory_record(id),
    origin              TEXT NOT NULL
...
CREATE TABLE attribute_resolution (
    ...
    field               TEXT NOT NULL,
    resolved_value      TEXT,
    winning_record_id   VARCHAR(26) REFERENCES inventory_record(id),
...
CREATE TABLE repository_attribute (
    repository_id       VARCHAR(26) NOT NULL REFERENCES repository(id),
    key                 TEXT NOT NULL,
    value_type          TEXT NOT NULL
                        CHECK (value_type IN ('text','number','datetime','bool')),
    value_text          TEXT,
...
CREATE TABLE repository_mapping (
    id                  VARCHAR(26) PRIMARY KEY,
    repository_id       VARCHAR(26) NOT NULL REFERENCES repository(id),
    repo_scope_id       VARCHAR(26) REFERENCES repo_scope(id),
    application_id      VARCHAR(26) NOT NULL REFERENCES application(id),
    managed_entity_id   VARCHAR(26) REFERENCES managed_entity(id),
    decision_id         VARCHAR(26) REFERENCES decision(id),
    created_by          TEXT NOT NULL,
    created_at          TIMESTAMPTZ NOT NULL
);
...
CREATE TABLE build_artifact (
    ...
    pipeline_ref        TEXT,
    managed_entity_id   VARCHAR(26) REFERENCES managed_entity(id),   -- NULL = orphan (§7)
    mapping_decision_id VARCHAR(26) REFERENCES decision(id),
    content_hash        TEXT NOT NULL,
    first_seen          TIMESTAMPTZ NOT NULL,
    last_seen           TIMESTAMPTZ NOT NULL,
    absent_since        TIMESTAMPTZ,
...
CREATE TABLE orphan_disposition (
    id                  VARCHAR(26) PRIMARY KEY,
    subject_id          VARCHAR(26) NOT NULL UNIQUE REFERENCES subject(id),
...
CREATE TABLE audit_event (
    ...
    subject_id          VARCHAR(26) REFERENCES subject(id),   -- NULL for process-only events
    source_id           VARCHAR(26) REFERENCES source(id),    -- source setup and sync events
    before_text         TEXT,
    after_text          TEXT,
    rationale           TEXT,
    decision_id         VARCHAR(26) REFERENCES decision(id),
    decision_packet_id  VARCHAR(26) REFERENCES decision_packet(id),
    sync_run_id         VARCHAR(26) REFERENCES sync_run(id)
);
...
VALUES ('SYSTEM00000000000000000000', 'internal', 'internal', TRUE,
        'Internal Register',
